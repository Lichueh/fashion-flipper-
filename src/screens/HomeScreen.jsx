...
const communityPreviews = [
  ...
  { id: 2, user: 'Jason', emoji: '🧢', bg: 'from-orange-100 to-amber-50',   item: 'Vintage Bucket Hat', likes: 89  },
  { id: 3, user: 'Lily',  emoji: '🛍', bg: 'from-lime-100 to-green-50',     item: 'Plaid Shopper',      likes: 234 },
  { id: 4, user: 'Leo',   emoji: '🧢', bg: 'from-stone-100 to-orange-50',   item: 'Denim Hat',          likes: 67  },
]
      ...
      <div className="flex-1 overflow-y-auto pb-1">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-2xl font-bold text-primary-100 tracking-tight">Fashion Flipper</h1>
              </div>
              <p className="text-primary-100 text-xs mt-0.5">Give old clothes a new life</p>
            </div>
        ...
        <div className="mx-5 grid grid-cols-3 gap-2.5 mb-5">
          {[
            ...
            <div key={s.label} className="bg-primary-100 rounded-2xl p-3 text-center border border-primary-200 shadow-sm">
        ...
        <div className="mb-5">
          <div className="flex justify-between items-center px-5 mb-3">
            <h3 className="font-semibold text-secondary-100 text-sm">Community Picks</h3>
            <button
          ...
          <div className="flex gap-3 overflow-x-auto pb-1 px-5">
            {communityPreviews.map(p => (
              <div key={p.id} className="flex-shrink-0 w-28">
                ...
                <p className="text-xs font-medium text-secondary-100 truncate">{p.item}</p>
                <p className="text-[10px] text-secondary-100 mt-0.5">@{p.user} · ❤️ {p.likes}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-5 mb-6">
          <h3 className="font-semibold text-secondary-100 text-sm mb-3">Available Templates</h3>
          <div className="grid grid-cols-2 gap-3">
            {[
              { emoji: '👜', name: 'Tote Bag',   diff: 'Beginner',  time: '1.5 hrs', bg: 'bg-primary-50', border: 'border-primary-200' },
              { emoji: '🧢', name: 'Bucket Hat', diff: 'Beginner+', time: '2–3 hrs', bg: 'bg-primary-50', border: 'border-primary-200' },
            ].map(t => (
              <div
                key={t.name}
                onClick={() => navigate('upload')}
              ...
              >
                ...
                <p className="font-semibold text-secondary-800 mt-2 text-sm">{t.name}</p>
                <p className="text-[11px] text-secondary-700 mt-0.5">{t.diff} · ⏱ {t.time}</p>
              </div>
            ))}
          </div>
        </div>
