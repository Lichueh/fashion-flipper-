  ...
  return (
    <div className="h-full flex flex-col bg-primary-800">
      ...
      <div className="flex items-center px-5 pt-8 pb-2">
        ...
        <div>
          <h2 className="font-semibold text-primary-100">Choose Upcycling Template</h2>
          <p className="text-[11px] text-primary-100 mt-0.5">Recommended based on AI analysis</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-6 pt-4 space-y-4">
        ...
        {items.map((template, idx) => {
          ...
          return (
            ...
            >
              {/* Card header */}
              <div className={`px-5 pt-5 pb-4 ${isRecommended ? 'bg-primary-50' : ''}`}>
                {isRecommended && (
                ...
                <div className="flex items-center gap-4">
                  <div className={`w-16 h-16 rounded-2xl flex-shrink-0 overflow-hidden ${template.accentColor}`}>
                    {template.id === 'bag'
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-bold text-primary-900 text-lg">{template.name}</h3>
                      <span className={`text-sm font-bold ${template.matchScore >= 85 ? 'text-primary-600' : 'text-secondary-600'}`}>
                        {template.matchScore}%
                      </span>
                    </div>
              ...
              <div className="px-5">
                <div className="h-1.5 bg-primary-200 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${template.matchScore >= 85 ? 'bg-primary-400' : 'bg-secondary-300'}`}
              ...
              <div className="px-5 pt-3 pb-4">
                <p className="text-primary-700 text-sm leading-5 mb-3">{template.description}</p>
